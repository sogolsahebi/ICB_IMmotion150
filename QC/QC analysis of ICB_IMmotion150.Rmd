...
filtered_expression_data  <- as.data.frame(scale(filtered_expression_data))


 normalized_expr <- as.data.frame(lapply(filtered_expression_data, function(x) (x - min(x)) / (max(x) - min(x)) * 4 - 2))

# Define expression colors
expression_colors <- colorRamp2(c(-2, 0, 2), c("#FFD700", "white", "#6A0DAD"))
