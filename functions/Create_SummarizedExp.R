...
format_se <- function(assay, coldata, assay_type, convert_gene_name=TRUE, check_study=FALSE, is_isoform=FALSE){
  ...
  print(paste("line38:", dim(assay)[1], "rows by", dim(assay)[2], "columns."))
  ...
  if(assay_type == "snv"){
    ...
    if(is_isoform){
      ...
      print(paste("line135:", dim(features_df)[1], "rows by", dim(features_df)[2], "columns."))
    ...
    if (check_study){
      ...
      #TODO: delete this
      print("this shouldnt print1")
      
    }
    
    else{
      
      print(paste("line135 assay:", dim(assay)[1], "rows by", dim(assay)[2], "columns."))
      assay <- assay[rownames(assay) %in% rownames(features_df), ]
      #TODO:delete this 
      print(paste("line135:", dim(assay)[1], "rows by", dim(assay)[2], "columns."))
    ...
    assay <- assay[order(rownames(assay)), ]
    #TODO:delete
    print(paste("line142:", dim(assay)[1], "rows by", dim(assay)[2], "columns."))
    ...
    print(paste("line 162:", dim(assay_list[[assay_type]])[1], "rows by", dim(assay_list[[assay_type]])[2], "columns."))
...
Create_EXP_SummarizedExperiment = function( study, case , clin, expr, feat_snv, feat_cna, feat_cin, snv_bool, cna_bool, is_isoform=FALSE ){
  ...
  #add (TODO: add Ravi and IMmotion150 )
  study_with_gene_id <- c("Braun", "Gide", "Hugo", "INSPIRE", "Jung", "Kim", "Miao.1", "Nathanson", "Riaz", "Snyder", "Van_Allen", "VanDenEnde", "Ravi" , "IMmotion150")
  ...
  expr = expr[ , patient ]
  #TODO:delete this 
  print(paste("229:", dim(expr)[1], "rows by", dim(expr)[2], "columns."))
  ...
  if( expr_bool ){
    if(expr_with_counts_isoforms){
      ...
      for(expr_file in expr_files){
        ...
        #TODO:delete
        print(paste("349:", dim(expr)[1], "rows by", dim(expr)[2], "columns."))
